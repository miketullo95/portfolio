...
subtitle: "Owning end-to-end redesign of Yale's primary authentication gateway — driving behavioral change at scale for 35,000+ daily users through cross-functional execution with security, identity management, and engineering."
...
The Yale Single Sign-On (SSO) system is the primary authentication gateway for the entire Yale community — serving over 35,000 students, faculty, and staff every day. As the critical entry point to Yale's services, it had to be reliable, accessible, and trustworthy at institutional scale.

I owned the end-to-end redesign of the authentication flow — login screen, password reset, error handling, and account management — coordinating across IT security, identity management, and engineering to deliver a more reliable, accessible experience without introducing risk to the infrastructure underpinning 35,000+ daily users.
...
1. **Define Requirements** — Collaborated with IT security, identity management, and accessibility experts to establish technical and compliance constraints before touching the interface
2. **Flow Architecture** — Mapped user flows for all authentication paths, including every edge case and error state, to define the full behavior surface
3. **Wireframing** — Created low-fidelity wireframes focused on interaction patterns and recovery paths across device types
4. **Interface Specification** — Developed high-fidelity specifications aligned with Yale's brand and WCAG 2.1 AA compliance
5. **Prototyping** — Built interactive prototypes for user testing and cross-functional stakeholder review with security and engineering
6. **Testing & Iteration** — Conducted multiple rounds of usability testing and iterated based on findings across user roles and accessibility needs
7. **Implementation** — Partnered with engineering to verify production fidelity, accessibility compliance, and behavioral correctness across all authentication paths
...
{/* TODO: verify impact metric — Confirm the 38% reduction in help desk tickets with IT support data. Specific question: what was the absolute ticket volume before vs. after launch, and over what time window? */}
<StatBlock stats={[
...
- The authentication flow patterns were adopted as the campus standard across other Yale web applications
...
- Established a model for cross-functional delivery — security, identity management, engineering, and UX working from shared requirements rather than in sequence
