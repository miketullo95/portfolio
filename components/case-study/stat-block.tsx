...
export default function StatBlock({ stats = [] }: { stats: Stat[] }) {
  ...
  return (
    <ul
      ...
      className="not-prose grid grid-cols-1 sm:grid-cols-3 gap-6 my-12 p-8 bg-blue-50 dark:bg-blue-950/20 rounded-2xl border border-blue-100 dark:border-blue-900/50 list-none [&>li]:before:content-none [&>li]:pl-0"
    >
      {stats.map(({ value, label }) => (
        <li key={label} className="text-center list-none">
          ...
          <p
            ...
            className="font-display text-5xl font-black text-blue-600 dark:text-blue-400 mb-2 tracking-tight"
            aria-hidden="true"
          >
            {value}
          </p>
